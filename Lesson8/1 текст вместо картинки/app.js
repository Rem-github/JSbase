...
buttons.forEach(function (button) {
    button.addEventListener('click', clickedButtonEvent => handleClick(clickedButtonEvent));
});
...
function handleClick(clickedButtonEvent) {
    ...
    const card = {
        wrap: cardNode.querySelector('.product'), /* сначала я сделал так и в hideMoreText(card)
        мне пришлось доставать div с desc вот так card.productName.nextElementSibling.remove(); т.к. 
        wrap.querySelector не работал...
        Понимал, что криво и пошел смотреть решение. querySelector смотрит только внутри, поэтому не видит
        класс самого div c .product. Надо было либо брать document или cardNode.parentNode, или как в решении просто присвоить
        cardNode в wrap. Но оставляю как есть, мой код был именно такой :(
        */
...
function hideMoreText(card) {
    ...
    card.productName.nextElementSibling.remove();
